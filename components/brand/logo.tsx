import { cn } from "@/lib/utils";

/**
 * Glifo oficial Te Quiero (corazón + infinito) — extraído del logotipo.
 * Usa currentColor → pintable desde CSS.
 */
export function HeartMark({ className }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="395 0 285 245"
      className={className}
      aria-hidden="true"
    >
      <path
        fill="currentColor"
        d="M665.5,209.76l-12.23-12.23-19.61,19.61,12.23,12.23c34.42,34.42,78.2,55.57,125.6,61.14,3.72.44,9.72.77,15.36,1.01,7.81.33,14.32-5.93,14.32-13.75h0c0-7.4-5.85-13.39-13.24-13.75-4.45-.22-8.86-.59-13.24-1.11-41.21-4.91-79.27-23.23-109.19-53.14"
      />
      <path
        fill="currentColor"
        d="M671.26,24.02c-32.02-32.03-84.13-32.03-116.15,0l-96.54,96.54c-21.22,21.2-55.73,21.2-76.93,0-21.21-21.21-21.21-55.72,0-76.94,21.2-21.21,55.72-21.21,76.93,0l8.87,8.87c5.41,5.41,14.19,5.42,19.61,0h0c5.42-5.41,5.42-14.19,0-19.61l-8.87-8.87c-32.02-32.03-84.12-32.02-116.15,0-32.02,32.02-32.02,84.13,0,116.15,16.01,16.01,37.04,24.02,58.08,24.02s42.07-8.01,58.08-24.02l52.72-52.72,43.82-43.82c21.21-21.21,55.72-21.21,76.93,0,21.21,21.21,21.21,55.72,0,76.94l-37.68,37.68-8.85-8.85c-5.41-5.41-14.19-5.41-19.61,0h0c-5.41,5.41-5.41,14.19,0,19.61l8.85,8.85-77.73,77.72-67.9-67.9c-9.28,2.51-18.85,3.88-28.64,3.88-2.31,0-4.61-.08-6.91-.22l103.45,103.45,154.62-154.61c32.03-32.02,32.03-84.13,0-116.15"
      />
    </svg>
  );
}

/**
 * Wordmark oficial Te Quiero — paths exactos del archivo de marca.
 * Usa currentColor → cambia el color con className text-* (white, tq-sky, tq-ink…).
 */
export function Wordmark({
  className,
}: {
  className?: string;
}) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 1544.78 294.78"
      className={cn("h-7 w-auto select-none", className)}
      aria-label="Te Quiero"
      role="img"
    >
      <g fill="currentColor">
        <path d="M209.12,66.5v50.2h47.62c6.32,0,11.44,5.12,11.44,11.44h0c0,6.32-5.12,11.44-11.44,11.44h-47.62v52.41h55c6.32,0,11.44,5.12,11.44,11.44h0c0,6.32-5.12,11.44-11.44,11.44h-83.05V43.62h83.05c6.32,0,11.44,5.12,11.44,11.44h0c0,6.32-5.12,11.44-11.44,11.44h-55Z" />
        <path d="M770.05,43.86c7.75,0,14.03,6.28,14.03,14.03v94.99c0,12.96,3.4,22.72,10.21,29.28,6.8,6.56,16.28,9.84,28.42,9.84s21.86-3.28,28.67-9.84c6.8-6.56,10.21-16.32,10.21-29.28V57.89c0-7.75,6.28-14.03,14.03-14.03h0c7.75,0,14.03,6.28,14.03,14.03v94.5c0,13.95-3.04,25.76-9.1,35.43-6.07,9.69-14.19,16.9-24.36,21.66-10.17,4.76-21.41,7.14-33.71,7.14s-23.5-2.38-33.59-7.14c-10.09-4.75-18.09-11.97-23.99-21.66-5.91-9.68-8.86-21.49-8.86-35.43V57.89c0-7.75,6.28-14.03,14.03-14.03h0Z" />
        <path d="M1068.3,66.5v50.2h47.62c6.32,0,11.44,5.12,11.44,11.44h0c0,6.32-5.12,11.45-11.44,11.45h-47.62v52.41h55c6.32,0,11.44,5.12,11.44,11.44h0c0,6.32-5.12,11.44-11.44,11.44h-83.05V43.62h83.05c6.32,0,11.44,5.12,11.44,11.44h0c0,6.32-5.12,11.44-11.44,11.44h-55Z" />
        <path d="M1413.49,205.42c-13.37-7.47-23.96-17.88-31.75-31.25-7.79-13.38-11.69-28.43-11.69-45.16s3.9-31.74,11.69-45.03c7.79-13.29,18.37-23.66,31.75-31.13,13.37-7.47,28.01-11.2,43.92-11.2s30.8,3.73,44.17,11.2c13.37,7.46,23.91,17.84,31.63,31.13,7.7,13.29,11.57,28.3,11.57,45.03s-3.86,31.78-11.57,45.16c-7.72,13.37-18.25,23.79-31.63,31.25-13.37,7.46-28.1,11.2-44.17,11.2s-30.56-3.73-43.92-11.2M1487.68,184.5c8.86-5.17,15.79-12.55,20.79-22.15,5-9.6,7.5-20.71,7.5-33.35s-2.5-23.71-7.5-33.22c-5.01-9.51-11.94-16.82-20.79-21.9-8.86-5.08-18.95-7.63-30.27-7.63s-21.41,2.55-30.27,7.63c-8.86,5.09-15.79,12.39-20.79,21.9-5.01,9.51-7.5,20.59-7.5,33.22s2.5,23.75,7.5,33.35c5.01,9.6,11.94,16.98,20.79,22.15,8.86,5.16,18.95,7.75,30.27,7.75s21.41-2.59,30.27-7.75" />
        <path d="M950.67,71.73v129.14c0,7.75,6.28,14.03,14.03,14.03h0c7.75,0,14.03-6.28,14.03-14.03V43.86h-.19l-27.86,27.86Z" />
        <path d="M119.35,55.18h0c0,6.32-5.12,11.44-11.44,11.44h-34.08v134.12c0,7.75-6.28,14.03-14.03,14.03h0c-7.75,0-14.03-6.28-14.03-14.03V66.62H11.44c-6.32,0-11.44-5.12-11.44-11.44h0c0-6.32,5.12-11.44,11.44-11.44h96.47c6.32,0,11.44,5.12,11.44,11.44Z" />
        <path d="M1229.93,215.85l27.56-27.56,23.5,23.5c2.6,2.6,6.13,4.06,9.8,4.06h0c12.35,0,18.54-14.93,9.8-23.67l-23.5-23.5,22.54-22.54c17.71-17.71,22.76-43.08,13.17-66.22-9.58-23.13-31.09-37.5-56.12-37.5h-60.23v173.42h33.47ZM1224.19,70.16h32.48s0,0,0,0c16.27,0,26.43,10.53,30.51,20.39,4.09,9.85,4.35,24.49-7.15,35.99l-55.84,55.84v-112.21Z" />
        <path d="M665.5,209.76l-12.23-12.23-19.61,19.61,12.23,12.23c34.42,34.42,78.2,55.57,125.6,61.14,3.72.44,9.72.77,15.36,1.01,7.81.33,14.32-5.93,14.32-13.75h0c0-7.4-5.85-13.39-13.24-13.75-4.45-.22-8.86-.59-13.24-1.11-41.21-4.91-79.27-23.23-109.19-53.14" />
        <path d="M671.26,24.02c-32.02-32.03-84.13-32.03-116.15,0l-96.54,96.54c-21.22,21.2-55.73,21.2-76.93,0-21.21-21.21-21.21-55.72,0-76.94,21.2-21.21,55.72-21.21,76.93,0l8.87,8.87c5.41,5.41,14.19,5.42,19.61,0h0c5.42-5.41,5.42-14.19,0-19.61l-8.87-8.87c-32.02-32.03-84.12-32.02-116.15,0-32.02,32.02-32.02,84.13,0,116.15,16.01,16.01,37.04,24.02,58.08,24.02s42.07-8.01,58.08-24.02l52.72-52.72,43.82-43.82c21.21-21.21,55.72-21.21,76.93,0,21.21,21.21,21.21,55.72,0,76.94l-37.68,37.68-8.85-8.85c-5.41-5.41-14.19-5.41-19.61,0h0c-5.41,5.41-5.41,14.19,0,19.61l8.85,8.85-77.73,77.72-67.9-67.9c-9.28,2.51-18.85,3.88-28.64,3.88-2.31,0-4.61-.08-6.91-.22l103.45,103.45,154.62-154.61c32.03-32.02,32.03-84.13,0-116.15" />
      </g>
    </svg>
  );
}

/**
 * Mark cuadrado para sidebar colapsado / app icon.
 */
export function MarkBadge({
  className,
  variant = "primary",
}: {
  className?: string;
  variant?: "primary" | "ink" | "outline";
}) {
  const styles = {
    primary: "bg-tq-sky text-white",
    ink: "bg-tq-ink text-white",
    outline: "bg-transparent border border-white/30 text-white",
  };
  return (
    <div
      className={cn(
        "rounded-xl flex items-center justify-center shadow-sm overflow-hidden",
        styles[variant],
        className,
      )}
    >
      <HeartMark className="w-3/5 h-3/5" />
    </div>
  );
}
